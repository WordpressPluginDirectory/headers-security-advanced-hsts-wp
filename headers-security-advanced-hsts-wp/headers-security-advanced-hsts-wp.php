...
const HSTS_PLUGIN_VERSION = '5.1.1';
...
function hsts_plugin_get_headers( ?array $headers = null ): array {
        if ( $headers === null ) {
            $headers = array();
        }
...
function hsts_plugin_update_htaccess(): void {
    ...
    foreach ( hsts_plugin_get_headers() as $name => $header ) {
        $escaped_header = addslashes($header);
		$lines[] = 'Header set ' . $name . ' "' . $escaped_header . '"';
    }
...
function hsts_plugin_delete_old_options(): void {
    // Last referenced by plugin version 5.1.1.
